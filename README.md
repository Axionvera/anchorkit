...
Read the [public roadmap and full capability disclaimer](./docs/ROADMAP.md)
...
See `docs/LOCAL_VERIFICATION.md` for details and the optional `pnpm verify:full` variant (examples, package boundaries, Soroban contract tests).
...
2. **Secret keys are redacted everywhere.** Logs, errors, UI, and even many
   ...
   See `docs/SECRET_KEY_HANDLING.md` for the R0–R6 rules enforced on every PR.
...
* A pre-merge review checklist every PR must pass: `docs/MAINTAINER_REVIEW_CHECKLIST.md`.
* A labels manifest covering Stellar ecosystem taxonomy plus the campaign labels: see `.github/LABELS.yml`.

## 💬 Community

* Stellar developers: visit [developers.stellar.org](https://developers.stellar.org) and the
  [Stellar Discord](https://discord.gg/stellardev) for general questions.
* AnchorKit-specific questions: file an issue using the templates in `.github/ISSUE_TEMPLATE/`.
* Code of conduct: `CODE_OF_CONDUCT.md`.
* Private security disclosures: `SECURITY.md`.

## ⚖️ Licence

AnchorKit source code is released under the Apache 2.0 licence. See `LICENCE` for the full text.