...
| 3    | `pnpm test`         | Vitest (and other package tests) via turbo, including `@anchorkit/integration-tests` |
...
`pnpm test:integration` is the focused command for the
[`tests/integration`](./INTEGRATION_TESTS.md) workspace. It is also included
when you run `pnpm test`.

## Related docs

- [LOCAL_SETUP.md](./LOCAL_SETUP.md) — install prerequisites and run the dashboard
- [INTEGRATION_TESTS.md](./INTEGRATION_TESTS.md) — cross-package integration suite
